# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from freeapiapp import Freeapiapp, AsyncFreeapiapp
from tests.utils import assert_matches_type
from freeapiapp.types.social_media import BookmarkCreateResponse, BookmarkRetrieveResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestBookmarks:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: Freeapiapp) -> None:
        bookmark = client.social_media.bookmarks.create(
            "64ad9c97bab19994fc168932",
        )
        assert_matches_type(BookmarkCreateResponse, bookmark, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: Freeapiapp) -> None:
        response = client.social_media.bookmarks.with_raw_response.create(
            "64ad9c97bab19994fc168932",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        bookmark = response.parse()
        assert_matches_type(BookmarkCreateResponse, bookmark, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: Freeapiapp) -> None:
        with client.social_media.bookmarks.with_streaming_response.create(
            "64ad9c97bab19994fc168932",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            bookmark = response.parse()
            assert_matches_type(BookmarkCreateResponse, bookmark, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_create(self, client: Freeapiapp) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `post_id` but received ''"):
            client.social_media.bookmarks.with_raw_response.create(
                "",
            )

    @parametrize
    def test_method_retrieve(self, client: Freeapiapp) -> None:
        bookmark = client.social_media.bookmarks.retrieve()
        assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

    @parametrize
    def test_method_retrieve_with_all_params(self, client: Freeapiapp) -> None:
        bookmark = client.social_media.bookmarks.retrieve(
            limit="5",
            page="1",
        )
        assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

    @parametrize
    def test_raw_response_retrieve(self, client: Freeapiapp) -> None:
        response = client.social_media.bookmarks.with_raw_response.retrieve()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        bookmark = response.parse()
        assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

    @parametrize
    def test_streaming_response_retrieve(self, client: Freeapiapp) -> None:
        with client.social_media.bookmarks.with_streaming_response.retrieve() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            bookmark = response.parse()
            assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncBookmarks:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_create(self, async_client: AsyncFreeapiapp) -> None:
        bookmark = await async_client.social_media.bookmarks.create(
            "64ad9c97bab19994fc168932",
        )
        assert_matches_type(BookmarkCreateResponse, bookmark, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncFreeapiapp) -> None:
        response = await async_client.social_media.bookmarks.with_raw_response.create(
            "64ad9c97bab19994fc168932",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        bookmark = await response.parse()
        assert_matches_type(BookmarkCreateResponse, bookmark, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncFreeapiapp) -> None:
        async with async_client.social_media.bookmarks.with_streaming_response.create(
            "64ad9c97bab19994fc168932",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            bookmark = await response.parse()
            assert_matches_type(BookmarkCreateResponse, bookmark, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_create(self, async_client: AsyncFreeapiapp) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `post_id` but received ''"):
            await async_client.social_media.bookmarks.with_raw_response.create(
                "",
            )

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncFreeapiapp) -> None:
        bookmark = await async_client.social_media.bookmarks.retrieve()
        assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

    @parametrize
    async def test_method_retrieve_with_all_params(self, async_client: AsyncFreeapiapp) -> None:
        bookmark = await async_client.social_media.bookmarks.retrieve(
            limit="5",
            page="1",
        )
        assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncFreeapiapp) -> None:
        response = await async_client.social_media.bookmarks.with_raw_response.retrieve()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        bookmark = await response.parse()
        assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncFreeapiapp) -> None:
        async with async_client.social_media.bookmarks.with_streaming_response.retrieve() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            bookmark = await response.parse()
            assert_matches_type(BookmarkRetrieveResponse, bookmark, path=["response"])

        assert cast(Any, response.is_closed) is True
