# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from freeapiapp import Freeapiapp, AsyncFreeapiapp
from tests.utils import assert_matches_type
from freeapiapp.types.social_media.posts import ImageRemoveResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestImages:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_remove(self, client: Freeapiapp) -> None:
        image = client.social_media.posts.images.remove(
            image_id="64a3e45c9d02a1e3d365f08d",
            post_id="64a3e3bc9d02a1e3d365f06a",
        )
        assert_matches_type(ImageRemoveResponse, image, path=["response"])

    @parametrize
    def test_raw_response_remove(self, client: Freeapiapp) -> None:
        response = client.social_media.posts.images.with_raw_response.remove(
            image_id="64a3e45c9d02a1e3d365f08d",
            post_id="64a3e3bc9d02a1e3d365f06a",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        image = response.parse()
        assert_matches_type(ImageRemoveResponse, image, path=["response"])

    @parametrize
    def test_streaming_response_remove(self, client: Freeapiapp) -> None:
        with client.social_media.posts.images.with_streaming_response.remove(
            image_id="64a3e45c9d02a1e3d365f08d",
            post_id="64a3e3bc9d02a1e3d365f06a",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            image = response.parse()
            assert_matches_type(ImageRemoveResponse, image, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_remove(self, client: Freeapiapp) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `post_id` but received ''"):
            client.social_media.posts.images.with_raw_response.remove(
                image_id="64a3e45c9d02a1e3d365f08d",
                post_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `image_id` but received ''"):
            client.social_media.posts.images.with_raw_response.remove(
                image_id="",
                post_id="64a3e3bc9d02a1e3d365f06a",
            )


class TestAsyncImages:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_remove(self, async_client: AsyncFreeapiapp) -> None:
        image = await async_client.social_media.posts.images.remove(
            image_id="64a3e45c9d02a1e3d365f08d",
            post_id="64a3e3bc9d02a1e3d365f06a",
        )
        assert_matches_type(ImageRemoveResponse, image, path=["response"])

    @parametrize
    async def test_raw_response_remove(self, async_client: AsyncFreeapiapp) -> None:
        response = await async_client.social_media.posts.images.with_raw_response.remove(
            image_id="64a3e45c9d02a1e3d365f08d",
            post_id="64a3e3bc9d02a1e3d365f06a",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        image = await response.parse()
        assert_matches_type(ImageRemoveResponse, image, path=["response"])

    @parametrize
    async def test_streaming_response_remove(self, async_client: AsyncFreeapiapp) -> None:
        async with async_client.social_media.posts.images.with_streaming_response.remove(
            image_id="64a3e45c9d02a1e3d365f08d",
            post_id="64a3e3bc9d02a1e3d365f06a",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            image = await response.parse()
            assert_matches_type(ImageRemoveResponse, image, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_remove(self, async_client: AsyncFreeapiapp) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `post_id` but received ''"):
            await async_client.social_media.posts.images.with_raw_response.remove(
                image_id="64a3e45c9d02a1e3d365f08d",
                post_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `image_id` but received ''"):
            await async_client.social_media.posts.images.with_raw_response.remove(
                image_id="",
                post_id="64a3e3bc9d02a1e3d365f06a",
            )
