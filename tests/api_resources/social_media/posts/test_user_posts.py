...
class TestUserPosts:
    ...
    def test_method_list(self, client: Freeapiapp) -> None:
        user_post = client.social_media.posts.user_posts.list(
            username="santos82",
        )
        assert_matches_type(UserPostListResponse, user_post, path=["response"])
    ...
    def test_raw_response_list(self, client: Freeapiapp) -> None:
        response = client.social_media.posts.user_posts.with_raw_response.list(
            username="santos82",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        user_post = response.parse()
        assert_matches_type(UserPostListResponse, user_post, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: Freeapiapp) -> None:
        with client.social_media.posts.user_posts.with_streaming_response.list(
            username="santos82",
        ) as response:
    ...
    def test_path_params_list(self, client: Freeapiapp) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `username` but received ''"):
            client.social_media.posts.user_posts.with_raw_response.list(
                username="",
            )


class TestAsyncUserPosts:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_list(self, async_client: AsyncFreeapiapp) -> None:
        user_post = await async_client.social_media.posts.user_posts.list(
            username="santos82",
        )
        assert_matches_type(UserPostListResponse, user_post, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncFreeapiapp) -> None:
        response = await async_client.social_media.posts.user_posts.with_raw_response.list(
            username="santos82",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        user_post = await response.parse()
        assert_matches_type(UserPostListResponse, user_post, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncFreeapiapp) -> None:
        async with async_client.social_media.posts.user_posts.with_streaming_response.list(
            username="santos82",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
    ...
    async def test_path_params_list(self, async_client: AsyncFreeapiapp) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `username` but received ''"):
            await async_client.social_media.posts.user_posts.with_raw_response.list(
                username="",
            )
