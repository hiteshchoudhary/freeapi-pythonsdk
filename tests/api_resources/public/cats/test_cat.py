...
class TestCat:
    ...
    def test_method_random(self, client: Freeapiapp) -> None:
        cat = client.public.cats.cat.random()
        assert_matches_type(CatRandomResponse, cat, path=["response"])

    @parametrize
    def test_raw_response_random(self, client: Freeapiapp) -> None:
        response = client.public.cats.cat.with_raw_response.random()
    ...
    def test_streaming_response_random(self, client: Freeapiapp) -> None:
        with client.public.cats.cat.with_streaming_response.random() as response:
...
class TestAsyncCat:
    ...
    async def test_method_random(self, async_client: AsyncFreeapiapp) -> None:
        cat = await async_client.public.cats.cat.random()
        assert_matches_type(CatRandomResponse, cat, path=["response"])

    @parametrize
    async def test_raw_response_random(self, async_client: AsyncFreeapiapp) -> None:
        response = await async_client.public.cats.cat.with_raw_response.random()
    ...
    async def test_streaming_response_random(self, async_client: AsyncFreeapiapp) -> None:
        async with async_client.public.cats.cat.with_streaming_response.random() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
